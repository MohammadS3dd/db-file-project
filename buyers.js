...
buyersRouter.get("/", function (req, res) {
  res.json(getAll(DB_NAME));
});

buyersRouter.get("/:id", function (req, res) {
  const { id } = req.params;
  res.json(getById(DB_NAME, id));
});

buyersRouter.post("/", function (req, res) {
  const body = req.body;
  const dataToSave = {};
  const fields = ["name", "bookid"];
...
buyersRouter.post("/search-name/", function (req, res) {
  const { name } = req.body;
